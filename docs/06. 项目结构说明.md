...
- `feature/` 内不要新建 `data/`、`domain/`、`repository/`、`use_case/`。
...
│   │   ├── read_model/
│   │   └── use_case/
│   └── share/                           # 事务执行抽象等 application 共享能力
├── domain/                               # 领域层：按业务域分子目录
│   ├── ledger/
...
- 目录/文件名一律**单数形式**（`entity / valobj / service / port / command / query / use_case / read_model / feature / widget / page / view_model / provider / mapper / repository / migration / table` 等）。例外仅限 Drift / SQL 习惯（如 Drift 表类名 `Accounts / Transactions / Entries`、表注册集合等沿用复数）。
...
- **新增独立业务域**：按 08.1 的归类判断；先建 domain 规则和 port，再按业务复杂度决定是否补 application / infrastructure 完整结构。
