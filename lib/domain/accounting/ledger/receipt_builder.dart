...
class ReceiptBuilder {
  ...
  Failure? _validateTransferFee(CreateTransferCommand cmd) {
    ...
    if (fee.currency != cmd.amount.currency) {
  ...
  Future<Result<PostReceipt>> buildRefund(
    ...
    if (parent.currencyCode != cmd.amount.currency) {
      return const Result.failure(
        Failure(
    ...
    final refunded = await _query.getRefundedTotal(
      parent.rootTransactionId,
      currencyCode: parent.currencyCode,
    );
    final addback =
        selfPrimaryAddback ?? Money.zero(currency: parent.currencyCode);
  ...
  Future<Result<PostReceipt>> buildReimbursementReceipt(
    ...
    final advanceFailure = _validateAdvance(advance, cmd.amount.currency);
    ...
    final addback =
        selfPrimaryAddback ?? Money.zero(currency: cmd.amount.currency);
  ...
  Future<Result<PostReceipt>> buildReimbursementClose(
    ...
    if (outstandingOverride != null) {
      if (outstandingOverride.currency != actual.currency) {
        return const Result.failure(
          Failure(
  ...
  Future<Result<PostReceipt>> buildRepayment(
  ...
  Future<Result<PostReceipt>> buildOpeningBalance(
    ...
    if (account.currencyCode != cmd.amount.currency) {
      return const Result.failure(
        Failure(
  ...
  Future<Result<PostReceipt>> buildBalanceAdjustment(
    ...
    if (account.currencyCode != cmd.targetBalance.currency) {
      return const Result.failure(
        Failure(
    ...
    final amount = Money(
      ...
      currency: cmd.targetBalance.currency,
    );
    final accountDirection = directionForBalanceDelta(
      accountType: account.type,
