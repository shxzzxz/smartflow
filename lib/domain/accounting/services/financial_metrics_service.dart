...
class FinancialMetricsServiceImpl implements FinancialMetricsService {
  ...
  Stream<CashflowComparison> watchCashflowComparison(
    ...
    return _aggregate.watchChanges().asyncMap((_) async {
      ...
      return CashflowComparison(
        current: _toCashflowSummary(results[0], query.currencyCode),
        previousSamePeriod: _toCashflowSummary(results[1], query.currencyCode),
        previousFullPeriod: _toCashflowSummary(results[2], query.currencyCode),
      );
    });
  }
      ...
      return [
        for (final date in dates)
          DailyCashflowSummary(
            date: date,
            income: Money(
              minorUnits: byDay[date]![AccountType.income] ?? 0,
              currency: query.currencyCode,
            ),
            expense: Money(
              minorUnits: byDay[date]![AccountType.expense] ?? 0,
              currency: query.currencyCode,
            ),
          ),
      ];
  ...
  Stream<BalanceSheetComparison> watchBalanceSheetComparison(
    ...
    return _aggregate.watchChanges().asyncMap((_) async {
      ...
      return BalanceSheetComparison(
        current: _toBalanceSnapshot(results[0], query.currencyCode),
        previous: _toBalanceSnapshot(results[1], query.currencyCode),
      );
    });
  }
      ...
      return [
        for (var i = 0; i < months.length; i++)
          NetAssetTrendPoint(
            month: months[i],
            netAssets:
                _toBalanceSnapshot(cutoffs[i], query.currencyCode).netAssets,
          ),
      ];
    });
  }
  ...
  CashflowSummary _toCashflowSummary(
    Map<AccountType, int> result,
    String currencyCode,
  ) {
    return CashflowSummary(
      income: Money(
        minorUnits: result[AccountType.income] ?? 0,
        currency: currencyCode,
      ),
      expense: Money(
        minorUnits: result[AccountType.expense] ?? 0,
        currency: currencyCode,
      ),
    );
  }

  BalanceSheetSnapshot _toBalanceSnapshot(
    Map<AccountType, int> result,
    String currencyCode,
  ) {
    return BalanceSheetSnapshot(
      assets: Money(
        minorUnits: result[AccountType.asset] ?? 0,
        currency: currencyCode,
      ),
      liabilities: Money(
        minorUnits: result[AccountType.liability] ?? 0,
        currency: currencyCode,
      ),
    );
  }
