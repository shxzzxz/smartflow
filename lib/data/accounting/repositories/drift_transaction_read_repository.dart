...
class DriftTransactionReadRepository implements TransactionReadRepository {
  ...
  Stream<List<Transaction>> watchPage(TransactionListQuery query) {
    final select = _db.select(_db.transactions)
      ..where(
        (t) => applyTransactionScope(transactions: _db.transactions, scope: query.scope),
      );
    ...
    if (query.occurredFrom != null) {
      select.where((t) => t.occurredAt.isBiggerOrEqualValue(query.occurredFrom!));
    }
    if (query.occurredUntil != null) {
      select.where((t) => t.occurredAt.isSmallerThanValue(query.occurredUntil!));
    }
    if (query.accountId != null) {
      final accountSubquery = _db.selectOnly(_db.entries, distinct: true)
  ...
  Future<Map<int, Map<TransactionDetailType, int>>> aggregateChildDetailAmounts({
    ...
    final select = _db.selectOnly(_db.transactionDetails).join([
      innerJoin(
        _db.transactions,
    ...
    for (final row in rows) {
      ...
      result.putIfAbsent(rootId, () => <TransactionDetailType, int>{})[type] = sum;
    }
    return result;
  }
