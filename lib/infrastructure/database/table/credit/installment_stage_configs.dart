...
class InstallmentStageConfigs extends Table {
  ...
  IntColumn get initialRatePpm => integer().nullable()();
  ...
  List<String> get customConstraints => [
    ...
    'CHECK (initial_rate_ppm IS NULL OR initial_rate_ppm >= 0)',
    'CHECK ((initial_rate_ppm IS NULL) = (rate_period IS NULL))',
    'CHECK (first_date IS NULL OR end_date >= first_date)',
    ...
    "CHECK (stage_kind != 'deferment' OR (repayment_method IS NULL "
        ...
        'AND amount_algorithm IS NULL AND periods IS NULL AND initial_rate_ppm IS NULL '
        'AND end_principal_minor IS NULL AND fixed_amount_minor IS NULL '
        'AND fee_minor IS NULL AND first_date IS NULL '
        'AND accrual_start_date IS NULL AND tail_difference IS NULL AND in_period_repricing_policy IS NULL))',
    "CHECK (stage_kind != 'repayment' OR (repayment_method IS NOT NULL AND first_date IS NOT NULL AND tail_difference IS NOT NULL))",
