...
void main() {
  group('DriftTransactionReadRepository', () {
    ...
    test('watchPage 应用 scope.assetLiability(current only,不限 excluded)', () async {
      await _post(
        database,
        purpose: BusinessPurpose.dailyExpense,
        amount: 100000,
      ...
      final page = await repository.watchPage(
      ...
      final statsPage = await repository.watchPage(
        const TransactionListQuery(scope: TransactionScopeFilter.stats),
      ).first;
      // 仅 stats=false 的 1 笔
