...
void main() {
  group('DriftTransactionRunner', () {
    test('run commits on success', () async {
      ...
      final id = await runner.run<String>(() async {
    ...
    test('run rolls back and rethrows BusinessException', () async {
      final database = createTestDatabase();
      addTearDown(database.close);
      final runner = DriftTransactionRunner(database);
      final repository = DriftAccountRepository(database);
    ...
    test('run rolls back and rethrows unexpected exception', () async {
      final database = createTestDatabase();
      addTearDown(database.close);
      final runner = DriftTransactionRunner(database);
      final repository = DriftAccountRepository(database);
