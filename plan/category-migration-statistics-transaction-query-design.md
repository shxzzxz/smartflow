...
> 状态：主方案已落地；落地后审查发现待处理项，见第 7 节。
>
> 本文替代“归档分类用于查询归并”的方向；三部分主路径已实现并通过现有验证，审查发现的边界问题与测试缺口尚待处理。
...
1. **高：统计钻取遗漏子交易贡献。** 当前顶层列表筛选只匹配顶层交易自身分录；报销差额记录在 `REIMBURSEMENT_CLOSE` 子交易上，可能出现统计有金额、钻取无贡献交易。需要确定钻取返回顶层交易组还是实际子交易，并据此调整仓储匹配边界。
2. **中：列表主分类仍按第一个同类型分录推断。** 方案要求分类投影不能依赖任意分录顺序。需要确定稳定来源，兼顾历史/导入异常数据的退化行为。
3. **中：整批回滚与派生分录重写缺少真实验证。** 现有分类迁移测试使用透传事务执行器和记录式 edit fake，只验证用途分派与异常停止；需要数据库集成测试证明第二组失败时第一组回滚，并验证退款/报销派生分录同步。
4. **中：一级分类零总额仍可能返回。** 当前只过滤空统计项；有界时间窗口内跨期退款可使不同子分类正负抵消，产生 `total = 0` 但子项非零的一级节点。需要决定隐藏零总额组，或修正原方案关于该状态不可能出现的假设。
5. **低：提交范围混入无关变更。** 审查提交段包含首页设置菜单修复及另一份 plan 删除；后续实现按问题拆分提交。

### 7.3 工程规范问题

- `CategoriesViewModel.migrateTransactions` 缺少 ViewModel unit test。
- application 分类统计 read model 暴露 `totalMinor / amountMinor`，与非数据库金额统一使用 `Money` 的规范不一致。
- `TransactionAdjustmentKind` 的 label/tone 由两个重复 switch 维护。
- 首页两个设置控件重复 optimistic state 同步结构。
- `FinancialMetricsService` 使用 `results[0] / results[1]` 与运行时 cast，结果身份不清晰。

### 7.4 跨域待确认

`docs/domains/budget/预算业务设计.md` 要求分类删除或迁移时同步迁移预算，并在目标分类同月已有预算时合并金额。当前分类迁移与删除用例均未处理预算引用。预算能力落地前必须明确由哪个跨域 application 用例协调，不能通过删除文档规则规避该约束。
...
下一会话从第 7.2 节第 1 项“统计钻取遗漏子交易贡献”开始讨论；在业务语义确定前不修改实现。
